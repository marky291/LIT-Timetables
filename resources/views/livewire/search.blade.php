        ...
        <div x-on:click.away="open = false" class="flex flex-col w-full max-w-3xl min-h-0 mx-auto bg-white rounded-2xl" style="box-shadow: 0 25px 50px -12px rgba(0,0,0,.25)">
            ...
            <section class="px-6 overflow-auto" style="max-height: 50vh">
                ...
                @if ($results->count())
                    ...
                    @endforeach
                @else
                    @if ($recent->count() || $favorites->count())
                        @if ($recent->count())
                                ...
                                @foreach ($recent as $i => $model)
                                    <li class="relative mt-2">
                                        <div wire:key="recent-{{ $loop->index }}" class="flex items-center justify-between rounded-lg cursor-pointer hover:bg-indigo-500 hover:text-white" :class="{ 'bg-indigo-500 text-white': selected === {{$i}} }">
                                            ...
                                            <div class="pr-4 py-4" wire:click="favorite({{$model->id}})">
                                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                                            </div>
                                            <div class="pr-4 py-4" wire:click="delete({{$model->id}})">
                        ...
                        @if ($favorites->count())
                            ...
                            <ul class="list-none">
                                @foreach ($favorites as $i => $model)
                                    <li class="relative mt-2">
                                        <div wire:key="favorite-{{ $loop->index }}" class="flex items-center justify-between rounded-lg cursor-pointer hover:bg-indigo-500 hover:text-white" :class="{ 'bg-indigo-500 text-white': selected === {{$i}} }">
                                            <div class="flex flex-1 items-center py-4 pl-4" wire:click="favorite('{{ addslashes($model->favorable::class) }}', '{{ $model->favorable->id }}', '{{ $model->favorable->route }}')">
                                                <div class="mr-2">
                                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                                                </div>
                                                <div class="flex items-center h-8">
                                                    <p class="font-semibold overflow-ellipsis whitespace-nowrap">{{ $model->favorable->routeTitle }}</p>
                                                </div>
                                            </div>
                                            <div class="pr-4 py-4 cursor-pointer" wire:click="disliked({{$model->id}})" >
                                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                            </div>
                                        </div>
                                    </li>
