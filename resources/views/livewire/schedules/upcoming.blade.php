    ...
    <div wire:poll.60000ms class="my-6 overflow-hidden bg-white dark:bg-dark-panel border dark:border-dark-border rounded-lg shadow">
        ...
        <div class="grid grid-cols-4 gap-4 px-4 py-5 rounded sm:p-6">
            @if ($this->readyToLoad == false)
                <div class="p-4 col-span-4 flex items-center justify-center dark:text-dark-icon px-10 py-6">
                    ...
                    <h3 class="ml-4 text-xl text-center py-6">Loading remaining classes</h3>
                </div>
            @else
                @if(count($this->schedule))
                    @foreach ($this->schedule as $schedule)
                        <div class="relative col-span-4 md:col-span-2 lg:col-span-1 p-4 {{ $schedule->isCurrentTime() ? 'bg-green-100 text-green-800 font-medium' : 'bg-gray-100' }} border border-transparent dark:border-dark-border dark:bg-dark-background rounded">
                            ...
                            @endswitch
                            <p class="mb-4">
                                <span class="text-lg font-bold text-gray-800 dark:text-dark-blue">{{ $schedule->starting_date->format('g:iA') }} - {{ $schedule->ending_date->format('g:iA') }}</span>
                            ...
                            <p class="font-semibold mb-2 dark:text-dark-text">{{ $schedule->module->name }}</p>
                            ...
                            @else
                                ...
                                <span class="text-indigo-500 dark:text-dark-blue">{{ $schedule->room->door }}</span><br>
                            @endif

                            <p class="mt-4 text-gray-500 dark:text-dark-icon">
