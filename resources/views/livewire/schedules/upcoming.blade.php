    ...
    <div class="grid grid-cols-4 gap-4 px-4 py-5 rounded sm:p-6">
        @if(count($upcoming))
            @foreach ($upcoming as $schedule)
                <div class="relative col-span-4 md:col-span-2 lg:col-span-1 p-4 {{ $schedule->isCurrentTime() ? 'bg-green-100 text-green-800 font-medium' : 'bg-gray-100' }} border-2 border-gray-100 rounded">
                    @switch($schedule->type->name)
                        @case('Online Lab Lecture')
                            <span class="absolute right-4 flex items-center justify-center w-6 h-6 text-white bg-red-500 rounded-full ring-2 ring-white">
                        ...
                        @case('Practical')
                            <span class="absolute right-4 flex items-center justify-center w-6 h-6 text-white bg-green-500 rounded-full ring-2 ring-white">
                            ...
                            @break
                        @case('Online Lecture')
                            <span class="absolute right-4 flex items-center justify-center w-6 h-6 text-white bg-yellow-500 rounded-full ring-2 ring-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @case('Online Tutorial')
                            <span class="absolute right-4 flex items-center justify-center w-6 h-6 text-white bg-pink-500 rounded-full ring-2 ring-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @break
                        @default
                            <span class="absolute right-4 flex items-center justify-center w-6 h-6 text-white bg-blue-500 rounded-full ring-2 ring-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
