    ...
    <div class="grid grid-cols-4 gap-4 px-4 py-5 rounded sm:p-6">
        @if(count($today))
            @foreach ($today as $schedule)
                <div
                    class="col-span-4 md:col-span-2 lg:col-span-1 p-4 {{ $schedule->isCurrentTime() ? 'bg-green-100 text-green-800 font-medium' : 'bg-gray-100' }} border-2 border-gray-100 rounded">
